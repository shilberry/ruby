...
class Resolvers::LinksSearch
    ...
    #if a filter exists, then called apply_filter
    ...
    #apply_filter recursively loops through the OR branches

    def apply_filter(scope, value)
        #normalize filters from nested OR
        branches = normalize_filters(value).reduce { |a, b| a.or(b)}
        scope.merge branches
    end

    def normalize_filters(value, branches = [])
        #add SQL like conditions
        scope = Link.all
